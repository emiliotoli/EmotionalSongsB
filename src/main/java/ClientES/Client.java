...
public class Client implements MetodiControlli_Client {
    ...
    private static String nome, cognome, codiceFiscale, via, numeroCivico, cap, comune, provincia, email, userID,
            password, p2;
    ...
    /**
     * @author Emilio Daverio - Stefano Farina
     *         Crea una nuova istanza del client. Inizializza le interfacce del
     *         server a null.
     */
    public Client() {
    ...
    /**
     * @author Emilio Daverio - Stefano Farina
     *         Metodo che serve per avviare il client e stabilire una connessione al
     *         server utilizzando RMI.
     *
    ...
    /**
     * @author Emilio Daverio - Stefano Farina
     *         Metodo che ricerca le informazioni sulle canzoni in base all'autore e
     *         all'anno.
     *
     * @param autoreCanzone Autore della canzone da cercare
     * @param annoCanzone   Anno della canzone da cercare
     *
     * @return Una lista di oggetti Canzone che corrispondono all'autore e all'anno
     *         specificati
     *
     * @throws IOException  Se si verifica un errore di I/O durante la ricerca
     * @throws SQLException Se si verifica un errore di database SQL
     */
    public static ArrayList<Canzone> RicercaCanzoniAutoreAnno(String autoreCanzone, int annoCanzone)
        ...
        informazioniCanzoneAuoreAnno = (ArrayList<Canzone>) interfaceNonLoggato.ricercaCanzoneAutoreAnno(autoreCanzone,
                annoCanzone);
    ...
    /**
     * @author Emilio Daverio - Stefano Farina
     *         Metodo che visualizza le emozioni associate a una specifica canzone
     *         identificata dal titolo e dall'autore.
     *
     * @param titoloCanzone Titolo della canzone da cercare
     * @param autoreCanzone Autore della canzone da cercare
     *
     ...
     *
     * @throws SQLException    Se si verifica un errore di SQL durante la query al
     *                         database
     * @throws RemoteException Se si verifica un errore di comunicazione remota
     */
    public static ArrayList<Emozione> visualizzaEmozioniCanzone(String titoloCanzone, String autoreCanzone)
        ...
        if (titoloCanzone != null && autoreCanzone != null) {
            emozioniCanzone = (ArrayList<Emozione>) interfaceNonLoggato.visualizzaEmozioni(titoloCanzone,
                    autoreCanzone);
            ...
            if (!emozioniCanzone.isEmpty()) {
                ...
                for (Emozione emozione : emozioniCanzone) {
                    ...
                    System.out.println("la percentuale dell' emozione " + "'" + nomeEmozione + "'" + " è --> "
                            + percentualeEmozione);
    ...
    /**
     * @author Emilio Daverio - Stefano Farina
     *         Metodo che inserisce una nuova emozione associata a una canzone nel
     *         sistema.
     *
     * @param idUtente            Identificativo dell'utente che inserisce
     *                            l'emozione
     ...
     * @param punteggioEmozione   Punteggio associato all'emozione (deve essere
     *                            compreso tra 1 e 5)
     *
     * @return 0 se l'inserimento è riuscito con successo, altrimenti un codice di
     *         errore
     *
     * @throws IOException  Se si verifica un errore di I/O durante l'inserimento
     * @throws SQLException Se si verifica un errore di SQL durante l'inserimento
     *                      nel database
     */
    public static int inserisciNuovaEmozione(String idUtente, String nomeEmozione, String titoloCanzone,
                ...
                if (notaEmozione.length() <= 50) {
                    if (spiegazioneEmozione.length() <= 250) {
                        if (punteggioEmozione >= 1 && punteggioEmozione <= 5) {
                            emozioniPerCanzone = interfaceLoggato.inserisciEmozione(idUtente, nomeEmozione,
                                    titoloCanzone, autoreCanzone, notaEmozione, spiegazioneEmozione, punteggioEmozione);
                            return 0;
                        } else {
     ...
     *
     * @return 0 se la registrazione è riuscita con successo, altrimenti un codice
     *         di errore
     *
     * @throws NotBoundException Se si verifica un errore durante il binding RMI
     * @throws IOException       Se si verifica un errore di I/O durante la
     *                           registrazione
     * @throws SQLException      Se si verifica un errore di SQL durante la
     *                           registrazione nel database
     */
    public static int registrazione(String nome, String cognome, String codiceFiscale, String via, String numeroCivico,
            String cap, String comune, String provincia, String email, String userID, String password, String p2)
        ...
        // richiamo il medodo che ho nel servere per vedere se esiste l'utente e aspetto
        ...
        Utente utete = new Utente(nome, cognome, codiceFiscale, via, numeroCivico, cap, comune, provincia, email,
                userID, password);
        inserimentoRiuscito = interfaceNonLoggato.registrazione(utete);

        // inserimentoRiuscito=serInterfaccia.registrazione(nome,
        // cognome,codiceFiscale,via,numeroCivico,
        // cap,comune,provincia,email,userID,password);
    ...
    /**
     * @author Emilio Daverio - Stefano Farina
     *         Metodo che crea una nuova playlist con il nome specificato per un
     *         utente identificato da userID.
     *
     * @param nomePlaylist Nome della playlist da creare
     * @param userID       Identificativo dell'utente
     *
     * @return 0 se la creazione è riuscita con successo, 1 se il nome della
     *         playlist esiste già, -1 se la creazione non è riuscita
     *
    ...
    public static int creaPlayList(String nomePlaylist, String userID) throws IOException, SQLException {
        Registry registroLoggato = LocateRegistry.getRegistry("192.168.43.230", 8080);
        ...
        // richiamo il medodo che ho nel servere per vedere se esiste il nome della
        // playlist e aspetto la risposta del server
    ...
    /**
     * @author Emilio Daverio - Stefano Farina
     *         Metodo che visualizza le playlist associate a un utente identificato
     *         da userID.
     *
     * @param userID Identificativo dell'utente
     *
    ...
    /**
     * @author Emilio Daverio - Stefano Farina
     *         Metodo che elimina una playlist associata all' utente identificato da
     *         userID.
     *
     * @param nomePlaylist Nome della playlist da eliminare
     *
     * @return 0 se l'eliminazione è riuscita con successo, 1 se l'eliminazione non
     *         è riuscita, -1 se il nome della playlist non esiste
     *
     * @throws IOException  Se si verifica un errore di I/O durante l'operazione
     * @throws SQLException Se si verifica un errore di SQL durante l'operazione
     */
    ...
    /**
     * @author Emilio Daverio - Stefano Farina
     *         Metodo che aggiunge una canzone in una playlist specificata per un
     *         utente identificato da userID.
     *
     ...
     *
     * @return 0 se l'aggiunta è riuscita con successo, 1 se l'aggiunta non è
     *         riuscita, -1 se il nome della playlist non esiste
     *
     * @throws RemoteException Se si verifica un errore di comunicazione remota
     * @throws SQLException    Se si verifica un errore di SQL durante l'operazione
     */
    public static int aggiuntaCanzoniPlaylist(String nomePlaylist, String userID, String titoloCanzone,
        ...
        if (controlloNomePlaylist) {
            ...
            inserimentoCanzonePlaylist = interfaceLoggato.aggiuntaCanzoniPlaylist(nomePlaylist, userID, titoloCanzone,
                    autoreCanzone);
    ...
    /**
     * @author Emilio Daverio - Stefano Farina
     *         Metodo che elimina una canzone da una playlist specificata per un
     *         utente identificato da userID.
     *
     ...
     *
     * @return 0 se l'eliminazione è riuscita con successo, 1 se l'eliminazione non
     *         è riuscita, -1 se il nome della playlist non esiste, -3 se le
     *         informazioni sulla canzone non sono state trovate
     *
     * @throws RemoteException Se si verifica un errore di comunicazione remota
     * @throws SQLException    Se si verifica un errore di SQL durante l'operazione
     */
    public static int eliminaCanzoniPlaylist(String nomePlaylist, String userID, String titoloCanzone,
            String autoreCanzone) throws RemoteException, SQLException {
        accessoServerLoggato();
        if (interfaceNonLoggato == null) {
            System.out.println("L'interfaccia non è stata inizializzata correttamente");
        ...
        if (controlloNomePlaylist) {
            ...
            if (chechinfocanzone) {
                cancellazioneCanzonePlaylist = interfaceLoggato.eliminaCanzoniPlaylist(nomePlaylist, userID,
                        titoloCanzone, autoreCanzone);
    ...
    /**
     * @author Emilio Daverio - Stefano Farina
     *         Metodo che visualizza le canzoni associate a una playlist specificata
     *         per un utente identificato da userID.
     *
     ...
     * @throws SQLException    Se si verifica un errore di SQL durante l'operazione
     */
    public static ArrayList<Canzone> visualizzaCanzoniPlaylist(String idUtente, String nomePlaylist)
        ...
        controlloNomePlaylist = interfaceLoggato.checkNomePlaylist(nomePlaylist);
        if (controlloNomePlaylist) {
            infoCanzoniPlaylist = (ArrayList<Canzone>) interfaceLoggato.VisualizzaCanzoniPlaylist(nomePlaylist,
                    idUtente);
    ...
    /**
     * @author Emilio Daverio - Stefano Farina
     *         Metodo che effettua l'accesso al server quando l'utente non è
     *         loggato.
     *         Questo metodo stabilisce una connessione al server utilizzando il
     *         registro RMI sulla porta 1099
     *         e recupera l'interfaccia ServerInterfaceNonLoggato per interagire con
     *         il server.
     *
     * @throws RemoteException Se si verifica un errore di comunicazione remota
     *                         durante la connessione al server
     */
    private static void accessoServerNonLoggato() throws RemoteException {
        Registry registroNonLoggato = LocateRegistry.getRegistry("192.168.43.230", 8080);
     ...
     *         Questo metodo stabilisce una connessione al server utilizzando il
     *         registro RMI sulla porta 1099
     *         e recupera l'interfaccia ServerInterfaceLoggato per interagire con il
     *         server.
     *
     * @throws RemoteException Se si verifica un errore di comunicazione remota
     *                         durante la connessione al server
